...
const dictModos = ['modos', 'time attack', 'desafio diario', 'diario', 'modos de jogo', 'timeattack', '60 segundos', 'desafios', 'modo', 'game modes', 'survival', 'sobrevivencia', 'sobrevivência'];
const dictPowerups = ['powerups', 'vidas', 'escudo', 'tempo extra', 'itens', 'power up', 'power-up', 'shield', 'extra time', 'multiplicador', 'x2', 'ajuda', 'bonus', 'bônus', 'recuperar vida', 'curar'];
const dictXP = ['xp', 'nivel', 'nível', 'conquistas', 'ranking', 'progressão', 'experiencia', 'experiência', 'level', 'levels', 'trofeus', 'troféus', 'pontos', 'score', 'pontuação', 'subir de nivel', 'upar', 'progresso'];
const dictGraficos = ['graficos', 'gráficos', 'cenarios', 'animação', 'barco', 'visual', 'graphics', 'particles', 'particulas', 'partículas', 'cenário', 'cenario', 'design', 'estilo', 'game feel', 'transições', 'transicoes', 'animado', 'visualização'];
...
async function processTutorialStep(input) {
  ...
  if (state.tutorialStep === 1) {
    if (input === 'cima') {
      ...
      await addMessage('err', 'PARADOX >', state.mode === 'assistant' ? 'Ops! Se eu pedi para ir para CIMA, a resposta é apenas "cima". Tente de novo. No jogo real você teria 3 vidas para errar!' : 'Sério? Você errou a direção "CIMA"? Digita "cima", por favor. No jogo real você teria 3 vidas e powerups pra se salvar, mas aqui é só tutorial.');
    }
  }
    ...
    if (input.includes('cima') || input.includes('baixo') || input.includes('direita')) {
      ...
      await addMessage('err', 'PARADOX >', state.mode === 'assistant' ? 'Atenção! Você não pode ir para a esquerda se houver um "!". Tente direita, cima ou baixo. No jogo você teria 3 vidas!' : 'Seu compilador mental falhou? "NÃO ESQUERDA" significa qualquer coisa, menos esquerda. Tente de novo. No jogo real você teria escudos e tempo extra pra compensar seus erros.');
    }
  }
    ...
    if (input.includes('cima') || input.includes('baixo')) {
      ...
      await addMessage('err', 'PARADOX >', state.mode === 'assistant' ? 'Incorreto. O "||" pedia para você escolher entre Cima ou Baixo. Tente novamente com uma dessas palavras. No jogo você tem 3 vidas!' : 'Erro de sintaxe orgânica. O comando deu duas opções fáceis. Escolha "cima" ou "baixo" logo. No jogo real você teria powerups pra ajudar.');
    }
  }
    ...
    if (input === 'direita') {
      ...
      await addMessage('err', 'PARADOX >', state.mode === 'assistant' ? 'Quase! A direção precisava ser Direita e não podia ser Cima. A resposta exata que atende a tudo é "direita". Tente digitá-la. No jogo você teria 3 vidas!' : 'Reprovado em Lógica 101. A resposta óbvia é "direita". Digite isso. No jogo real você teria escudos e multiplicadores pra compensar sua incompetência.');
    }
  }
    ...
    if (input.includes('cima') || input.includes('baixo')) {
      ...
      await addMessage('err', 'PARADOX >', state.mode === 'assistant' ? 'Não exatamente. Se o "!" proibiu tanto a Esquerda quanto a Direita, só nos restam os movimentos verticais. Tente "cima" ou "baixo"! No jogo você tem 3 vidas!' : 'Pane no sistema. Se eu proibi o eixo horizontal inteiro, o que sobra? Escolha Cima ou Baixo, rápido. No jogo real você teria tempo extra pra pensar.');
    }
  }
}
...
const inputNormalizado = normalizarTexto(input); 
  ...
  if (dictHelp.includes(input)) {
    ...
    await addMessage('bot', 'PARADOX >', state.mode === 'sarcastic' ? 'Sete humanos precisaram se juntar para tentar fazer o jogo não travar: Arthur Mendes, Igor Navarro, Kaique Dias, Kaique Oliveira, Leonardo Pedrão, Luiz Felipe e Victor Ferreira. E para fazer a web, sobrou para Igor Navarro(que provavelmente passou o projeto inteiro comendo bolo), Kaique Dias e Kaique Oliveira. Vai aplaudir ou vai jogar o "desafio"?' : 'O jogo original ! IndexError em Python foi criado por uma grande equipe de 7 mentes: Arthur Mendes, Igor Navarro, Kaique Dias, Kaique Oliveira, Leonardo Pedrão, Luiz Felipe e Victor Ferreira. Já a arquitetura deste site e do meu terminal web foi feita pelos arquitetos da TupãStudios: Igor Navarro, Kaique Dias e Kaique Oliveira.');
  ...
  } else if (dictModos.some(word => inputNormalizado.includes(word))) {
    await addMessage('bot', 'PARADOX >', state.mode === 'sarcastic' ? 'Como vocês enjoam rápido, a equipe adicionou um modo Time Attack de 60 segundos e Desafios Diários. Assim você pode passar vergonha de formas diferentes todos os dias.' : 'Além do modo de sobrevivência infinito, agora temos o Time Attack, onde você tem 60 segundos para pontuar o máximo possível, e os Desafios Diários que mudam todos os dias do ano!');


  } else if (dictPowerups.some(word => inputNormalizado.includes(word))) {
    await addMessage('bot', 'PARADOX >', state.mode === 'sarcastic' ? 'Humanos são tão frágeis que os desenvolvedores tiveram que dar 3 Vidas, Escudos e Tempo Extra. É praticamente um modo "café com leite" para vocês não chorarem no primeiro erro.' : 'O jogo agora é mais tolerante! Você começa com 3 vidas e pode coletar Powerups pelo caminho, como Tempo Extra (+5s), Escudo contra erros e Multiplicadores x2.');


  } else if (dictXP.some(word => inputNormalizado.includes(word))) {
    await addMessage('bot', 'PARADOX >', state.mode === 'sarcastic' ? 'Adicionamos uma barra de XP colorida e troféus virtuais porque sabemos que o cérebro primata precisa de validação constante e recompensas brilhantes para continuar jogando.' : 'Temos um sistema de progressão completo! Cada acerto te dá XP para subir de nível, além de um sistema com diversas conquistas para desbloquear e rankings separados por dificuldade.');


  } else if (dictGraficos.some(word => inputNormalizado.includes(word))) {
    await addMessage('bot', 'PARADOX >', state.mode === 'sarcastic' ? 'O jogo agora tem partículas, transições suaves e um barco animado. Tudo isso pra disfarçar que você ainda está resolvendo problemas de lógica básica.' : 'Os visuais foram aprimorados com partículas, transições suaves, cenários dinâmicos e um barco animado para uma experiência imersiva!');
